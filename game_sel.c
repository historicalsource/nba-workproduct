...
#include "audits.h"	
#include "game.h"
#include "id.h"			// Graphic IDs
#include "select.h"		// Misc Japple Crap
#include "sndcalls.h"	// Sound calls
#include "transit.h"	// Transition handler
#include "game_sel.h"
#include "coin.h"

#include "optscrn.h"	// Flash graphics

#define IMAGEDEFINE	1
#include "gamesel.h"	// Game Selection Graphics
...
//	External Functions
//-------------------------------------------------------------------------------------------------
extern int set_coin_hold(int flag);
extern void change_alpha_cc(void * parent, int cc);
...
static tmu_ram_texture_t gamesel_textures[] = {
	...
	{"plrcrd00.wms",JAPPLE_BOX_TID},		// Logos
	{"plrcrd01.wms",JAPPLE_BOX_TID},		// Logos
...
{
	&flasha10, &flasha09, &flasha08, &flasha07,
	&flasha05, &flasha04, &flasha03, &flasha02, 
	&flasha01,
	&flasha02, &flasha03, &flasha04, &flasha05,
...
//	snd_stop_all();
	...
	// Play drum loop
	snd_scall_bank("intro1", 0, VOLUME4, 127, SND_PRI_SET|SND_PRI_TRACK0);
...
//	joy = pjoy = joy1 | joy2 | joy3 | joy4;
	...
	while(!done)
	{
		...
		if (joy != pjoy)
		{
			...
			{
				snd_scall_bank("intro1",1,VOLUME2,255,PRIORITY_LVL3);
				cursor = 1;
				alpha = MAX_FADE;
				alpha_speed = -4;
			...
			{
				snd_scall_bank("intro1",1,VOLUME2, 0,PRIORITY_LVL3);
				cursor = 0;
				alpha = MAX_FADE;
				alpha_speed = -4;
				delay_alpha = FADE_DELAY;
...
void write_game_info(void)
{
	...
	if (!fp)
	{
		fprintf(stderr, "Can not open file RELOAD.NFO\n");
	}
#endif
	if (fp)
	{
...
static void one_logo_sparkle(int *args)
{
	...
	flash_obj->w_scale = 0.8f;
	flash_obj->h_scale = 0.8f;
	generate_sprite_verts(flash_obj);

	for (cnt=1; cnt < (int)(sizeof(logo_flash_imgs)/sizeof(image_info_t *))-1; cnt++)
	{
		sleep(3);
