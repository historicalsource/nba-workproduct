...
#ifdef INCLUDE_SSID
char *ss_movie_c = "$Workfile: movie.c $ $Revision: 25 $";
#endif
...
#define	MAX_FRAME_BUFFERS	 	2	/* was 4 */
...
extern short _SecReads(int, void *, int);
extern partition_table_t	*ide_get_partition_table(void);
extern void flush_disk_queue(void);
...
static void movie_display_proc(int *args)
{
	...
	// Set the movie display process pointer
//	mdp = cur_proc;
...
static unsigned short				*conversion_buffer;
static unsigned short				*convert_buf_save;
static control_file_data_t			control;
static int							sectors_per_frame;
static int							last_frame_hold;
static int							frames = 0;
static int							movie_proc_done;
...
//static int movie_state = MOVIE_STATE_IDLE;
static int movie_state = MOVIE_STATE_DONE;
...
void movie_set_transparency(float percent)
{
	...
	/*
	setting these fields effectively does this before drawing the sprite 
	 
	grConstantColorValue(scale<<24); 
...
void movie_proc(int *args)
{
	...
	while(cur_mc)
	{
		while(cur_mc->number_of_frames > 0)
		{
			// Wait for key here...
#if 0
...
static int movie_init(char *basename[])
{
	...
	// Make sure this is no installed callback
	dio_mode(DIO_MODE_BLOCK);
	...
	{
#ifdef DEBUG
		fprintf(stderr, "Can not allocate memory for movie control node\r\n");
		lockup();
#else
		return(0);
#endif
	...
	// Create the sprites used to display the movie frames on screen
	// 2 sprites per movie frame
	mov_sprite[0] = _beginobj(NULL,&movie_part1, movie_x, movie_y, movie_z, 0);
	if(!mov_sprite[0])
	{
		fprintf(stderr, "movie_init(): Can not create first sprite\r\n");
		JFREE(convert_buf_save);
...
#ifndef CENTER
	mov_sprite[1] = _beginobj(NULL,&movie_part2, movie_x + movie_part1.w, movie_y, movie_z, 1);
#else
	mov_sprite[1] = _beginobj(NULL,&movie_part2, movie_x, movie_y, movie_z, 1);
#endif
	if(!mov_sprite[1])
	{
		fprintf(stderr, "movie_init(): Can not create second sprite\r\n");
...
#ifdef CENTER
mov_sprite[0]->y_ang_vel = 0.02755783f;
mov_sprite[1]->y_ang_vel = 0.02755783f;
mov_sprite[0]->z_ang_vel = 0.013778915f;
mov_sprite[1]->z_ang_vel = 0.013778915f;
//mov_sprite[0]->x_ang_vel = 0.0068894575f;
//mov_sprite[1]->x_ang_vel = 0.0068894575f;
set_veladd_mode(mov_sprite[0], 1);
set_veladd_mode(mov_sprite[1], 1);
#endif
...
//		increment_audit(TEX_LOAD_FAIL_AUD);
	...
	for(i = 0; i < MAX_FRAME_BUFFERS; i++)
	{
		mb.frame_buffer[i] = (int *)JMALLOC((sectors_per_frame + 1) * SECTOR_SIZE);
		if(!mb.frame_buffer[i])
		{
			fprintf(stderr, "movie_init(): Can not allocate memory for movie frame buffer: %d\r\n", i);
			i--;
...
//	mdp = cur_proc;
	...
	// Should the last frame be held for a minute ?
	if(last_frame_hold)
	{
		// Set the movie state to paused
		movie_state = MOVIE_STATE_HOLD;
