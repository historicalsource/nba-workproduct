...
static texture_remap xlate_08 = {1, {0, -1, -1, -1}, {2, -1, -1, -1}};
static texture_remap xlate_48 = {1, {1, -1, -1, -1}, {2, -1, -1, -1}};
static texture_remap xlate_08_40 = {2, {0, 1, -1, -1}, {2, 0, -1, -1}};
...
/* CREATURE CHARACTER */
#include "/video/nba/models/waist_uc.h"
#include "/video/nba/models/torso_uc.h"
#include "/video/nba/models/arm_uluc.h"
#include "/video/nba/models/arm_uruc.h"
#include "/video/nba/models/arm_lluc.h"
#include "/video/nba/models/arm_lruc.h"
#include "/video/nba/models/hand_ruc.h"
#include "/video/nba/models/hand_luc.h"
#include "/video/nba/models/leg_uluc.h"
#include "/video/nba/models/leg_uruc.h"
#include "/video/nba/models/leg_lluc.h"
#include "/video/nba/models/leg_lruc.h"
#include "/video/nba/models/foot_ruc.h"
#include "/video/nba/models/foot_luc.h"
static LIMB *creature_limbs[] = {
	&limb_waist_uc,
	&limb_torso_uc,
	&limb_arm_uruc,
	&limb_arm_lruc,
...
static texture_remap *creature_limbs_map[] = {
	&xlate_08_40,			/* waist */
	&xlate_08_40,			/* torso */
	NULL,					/* arm UR */
	NULL,					/* arm LR */
...
static mascot_map name_list[] = {{"REDUCED_BIG_PLAYER", "rb_plyr", plyr_limbs_rm, plyr_limbs_rm_map},
								 ...
								 {"CREATURE_CHARACTER", "cftbl", creature_limbs, creature_limbs_map}
};
...
int main(int argc, char *argv[])
{
	...
	for (i = JOINT_PELVIS; i <= JOINT_LANKLE; i++) {
		if (i != JOINT_NECK) {
			...
			if (the_limb->strip != NULL) {
				strip_info = the_limb->strip->strip_info;
				while (strip_info->num_verts >= 0) {
					if (strip_info->text >= 0)
						strip_info->text = xlate_texture(mascot_index, i, strip_info->text);
					strip_info++;
				}
			}
			...
			/* write the STRIP_INFO list */
			fwrite(&the_limb->strip->num_strip_info, sizeof(the_limb->strip->num_strip_info), 1, outfile);
			fwrite( the_limb->strip->strip_info, sizeof(STRIP_INFO), the_limb->strip->num_strip_info, outfile);
			
			/* write the STRIP_VERTS list */
			fwrite(&the_limb->strip->num_strip_verts, sizeof(the_limb->strip->num_strip_verts), 1, outfile);
			fwrite( the_limb->strip->strip_verts, sizeof(STRIP_VERTS),the_limb->strip->num_strip_verts, outfile);

			/* write the BBOX_LIST list */
			if(the_limb->bbox == NULL)
				{
				num_bbox = 0;
				fwrite(&num_bbox, sizeof(num_bbox), 1, outfile);
				}
			else
				{			
				num_bbox = 8;
				fwrite(&num_bbox, sizeof(num_bbox), 1, outfile);
				fwrite(the_limb->bbox, sizeof(VERTEX),num_bbox, outfile);
				}

//				printf("%s  num_bbox: %d\n",bod_filename,num_bbox);
