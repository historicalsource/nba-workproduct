...
{CLEAR_PLAYER_RECORDS_ADJ, 0, 1, 1},
{CLEAR_WORLD_RECORDS_ADJ, 0, 1, 1},
{PLAYER_HEAD_SIZE_ADJ, 0, 1, 0},
{TOURNAMENT_MODE_ADJ, 0, 1, 0},
{COMPUTER_ASSISTANCE_ADJ, 0, 1, 1},
};
...
static void write_adjust_file(void)
{
	...
	ADD_YES_NO_ADJUST(fp, CLEAR_PLAYER_RECORDS_ADJ, "CLEAR PLAYER RECORDS", nfl_adjustment_info);
	ADD_YES_NO_ADJUST(fp, CLEAR_WORLD_RECORDS_ADJ, "CLEAR WORLD RECORDS", nfl_adjustment_info);

	ADD_LIST_ADJUST(fp, PLAYER_HEAD_SIZE_ADJ, "PLAYER HEAD SIZE", nfl_adjustment_info);
	ADJUST_LIST_NAME(fp, "HEAD SIZE");
	ADJUST_LIST_ITEM(fp, "NORMAL");
	ADJUST_LIST_ITEM(fp, "BIG");
	LIST_ADJUST_DONE(fp);

	ADD_ENABLE_DISABLE_ADJUST(fp, TOURNAMENT_MODE_ADJ, "TOURNAMENT MODE", nfl_adjustment_info);

	ADD_ENABLE_DISABLE_ADJUST(fp, COMPUTER_ASSISTANCE_ADJ, "COMPUTER ASSISTANCE", nfl_adjustment_info);
