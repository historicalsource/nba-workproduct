...
// Functions
extern void attract_loop(void);
extern void wait_for_any_butn_press(int, int);
extern void disc_flash_proc(int *args);
...
//	{"plqfsh.wms",JAPPLE_BOX_TID},				// 256 * 256 * 16bits
	...
	{ "sw_pn.wms", JAPPLE_BOX_TID },			// Sweeps PN
	{NULL,0}
};
...
void sweeps_proc(int *args)
{
	...
	qcreate("sweeps", JUMP_METER_PID, get_sweeps_number, 0, 0, 0, 0);

	while (!done)
	{
		sleep(1);
		if (!existp(JUMP_METER_PID, 0xffffffff))
			done = 1;
	}
...
//			flash_cursor(focus, JAPPLE_BOX_TID);
			qcreate("dsc_flsh",SPFX_PID,disc_flash_proc,
//						focus->x,
//						focus->y,
						PLATE_X - (focus->ii->ax) + (focus->ii->w/2),
						PLATE_Y + (focus->ii->ay + (focus->ii->h/2)),
						0, 0);
		}
		print_instring(STR_X, STR_Y, 5.0f, NUMBER_SID);
		sleep(1);
